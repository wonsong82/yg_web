...
/** READ THE STORE SETTINGS **/
		...
		private function _load_settings(){
			...
			$music_store_settings = array(
				...
				'ms_items_page' 			=> get_option( 'ms_items_page', MS_ITEMS_PAGE ),
				'ms_layout' 				=> get_option( 'ms_layout' ),
				
				'ms_paypal_email'			=> get_option( 'ms_paypal_email', MS_PAYPAL_EMAIL ),
				'ms_paypal_button' 			=> get_option( 'ms_paypal_button', MS_PAYPAL_BUTTON ),
				'ms_paypal_currency' 		=> get_option('ms_paypal_currency', MS_PAYPAL_CURRENCY ),
				'ms_paypal_currency_symbol' => get_option( 'ms_paypal_currency_symbol', MS_PAYPAL_CURRENCY_SYMBOL ),
				'ms_paypal_language' 		=> get_option( 'ms_paypal_language', MS_PAYPAL_LANGUAGE ),
				...
				'ms_old_download_link' 		=> get_option( 'ms_old_download_link', MS_OLD_DOWNLOAD_LINK ),				
				'ms_downloads_number' 		=> get_option( 'ms_downloads_number', MS_DOWNLOADS_NUMBER ),				
				...
				'ms_license_for_regular' 	=> get_option( 'ms_license_for_regular', '' ),
				'ms_license_for_free' 		=> get_option( 'ms_license_for_free', '' )
			);	
		} // End _load_settings
		...
		*/
		function settings_page(){
			...
			if ( isset( $_POST['ms_settings'] ) && wp_verify_nonce( $_POST['ms_settings'], plugin_basename( __FILE__ ) ) ){
				...
				// Licenses
				update_option('ms_license_for_regular', trim($_POST['ms_license_for_regular']));
				update_option('ms_license_for_free', trim($_POST['ms_license_for_free']));
...
<?php				
			...
			$current_tab = (isset($_REQUEST['tab']) && in_array($_REQUEST['tab'], array( 'song', 'collection', 'reports', 'settings', 'importer' ))) ? $_REQUEST['tab'] : (($_REQUEST['page'] == 'music-store-menu-reports') ? 'reports' : 'settings');
...
?>
						...
						<div class="inside">
							<table class="form-table">
								<tr valign="top">
									...
									<td>
										<input type="text" name="ms_main_page" size="40" value="<?php echo esc_attr($music_store_settings[ 'ms_main_page' ]); ?>" />
										<br />
								...
								<tr valign="top">
									...
									<td><input type="text" name="ms_items_page" value="<?php echo esc_attr($music_store_settings[ 'ms_items_page' ]); ?>" /></td>
								</tr>
								<tr valign="top">
					...
					<div class="postbox">
						...
						<table class="form-table">
							...
							<td><input type="text" name="ms_old_download_link" value="<?php echo esc_attr($music_store_settings[ 'ms_old_download_link' ]); ?>" /> <?php _e('day(s)', MS_TEXT_DOMAIN)?></td>
							</tr>
							
                            <tr valign="top">
							<th scope="row"><?php _e('Number of downloads allowed by purchase', MS_TEXT_DOMAIN); ?></th>
							<td><input type="text" name="ms_downloads_number" value="<?php echo esc_attr($music_store_settings[ 'ms_downloads_number' ]); ?>" /></td>
							</tr>  
						...
						<div class="inside">
							...
							<div style="float:left;margin-right:20px;">
								...
								<?php
									...
									{
										print '<option value="'.esc_attr($key).'"'.( ( isset( $group_by ) && $group_by == $key ) ? ' SELECTED' : '' ).'>'.$value.'</option>';
									}
								?>
								</select>
							</div>	
								...
								if(count($purchase_list)){	
									print '
										<div>
											...
											<label style="margin-right: 20px;" ><input type="checkbox" onclick="ms_load_report(this, \'sales_by_product\', \''.__( 'Sales by product', MS_TEXT_DOMAIN ).'\', \'product_name\', \'Bar\', \'post_title\', \'sum\');" /> '.__( 'Sales by product', MS_TEXT_DOMAIN ).'</label>
											<label><input type="checkbox" onclick="ms_load_report(this, \'download_by_product\', \''.__( 'Downloads free products', MS_TEXT_DOMAIN ).'\', \'download_by_product\', \'Pie\', \'post_title\', \'count\');" /> '.__( 'Downloads free products', MS_TEXT_DOMAIN ).'</label>
										</div>';
								}
							?>
									...
									<td align="right">
										<table>
											<tr>
												...
												<td>
													<input type="submit" value="<?php _e( 'Replace', MS_TEXT_DOMAIN ); ?>" class="button-primary" />
												</td>
											</tr>
										</table>
									</td>
...
/** LOADING PUBLIC OR ADMINSITRATION RESOURCES **/		
		...
		*/
		function public_resources(){
			...
			wp_localize_script('ms-media-script', 'ms_global', array('hurl' => MS_H_URL, 'play_all' => $play_all));
			...
			if ( $hook == 'post-new.php' || $hook == 'post.php' || $hook == 'index.php') {
				...
				if(isset($post) && $post->post_type == "ms_song"){
					...
					foreach($genre_list as $genre){
							$tags .= '<option value="'.esc_attr($genre->term_id).'">'.$genre->name.'</option>';
					}
					...
					foreach($artist_list as $artist){
							$tags .= '<option value="'.esc_attr($artist->term_id).'">'.$artist->name.'</option>';
					}
					...
					foreach($album_list as $album){
							$tags .= '<option value="'.esc_attr($album->term_id).'">'.$album->name.'</option>';
					}
			...
			if(
				...
				if($allow_filter_by_genre){
					...
					foreach($genres as $genre_item){
						$header .= "<option value='".esc_attr($genre_item->slug)."' ".(($genre == $genre_item->slug || $genre == $genre_item->term_id) ? "SELECTED" : "").">".$genre_item->name."</option>";
					}
					$header .= "</select></span>";
				}
                ...
                if($allow_filter_by_album){
					...
					foreach($albums as $album_item){
						$header .= "<option value='".esc_attr($album_item->slug)."' ".(($album == $album_item->slug || $album == $album_item->term_id ) ? "SELECTED" : "").">".$album_item->name."</option>";
					}
					$header .= "</select></span>";
				}
				...
				if($allow_filter_by_artist){
					...
					foreach($artists as $artist_item){
						$header .= "<option value='".esc_attr($artist_item->slug)."' ".(($artist == $artist_item->slug || $artist == $artist_item->term_id ) ? "SELECTED" : "").">".$artist_item->name."</option>";
					}
					$header .= "</select></span>";
				}
				$header .="</div>";
