	...
	class MSSong{
		...
		*/
		...
		function display_content($mode, $tpl_engine, $output='echo'){
            ...
            $ms_main_page = $music_store_settings[ 'ms_main_page' ];
			...
			if(!empty($this->file)){
                if($music_store_settings[ 'ms_paypal_enabled' ] && $music_store_settings[ 'ms_paypal_email' ] && !empty($this->price)){
					...
					{	
						...
						$song_arr['salesbutton'] = '<form action="'.MS_H_URL.'" method="post"><input type="hidden" name="ms-action" value="buynow" /><input type="hidden" name="ms_product_type" value="single" /><input type="hidden" name="ms_product_id" value="'.esc_attr($this->id).'" /><input type="image" src="'.esc_url($paypal_button).'" style="padding-top:5px;" /></form>';
					}	
                }elseif(
				...
				{
                    $song_arr['salesbutton']  = '<a href="'.$this->file.'" target="_blank" data-id="'.$this->id.'" class="ms-download-link">'.__('Download', MS_TEXT_DOMAIN).'</a>'.( ( !empty( $music_store_settings[ 'ms_license_for_free' ] ) ) ? '|<a href="'.$music_store_settings[ 'ms_license_for_free' ].'" target="_blank">'.__( 'License',  MS_TEXT_DOMAIN ).'</a>' : '' );
                }
			}
		...
		*/
		public static function print_metabox(){
			...
			{
				echo '<div class="music-store-error-mssg">'.implode( '<br>', $_SESSION[ 'ms_errors' ] ).'</div>';
			...
			echo '
				<table class="form-table product-data">
					...
					<tr>
						...
						<td>
							<input type="text" name="ms_year" id="ms_time" value="'.(($data && $data->year) ? esc_attr($data->year) : '').'" /> <span class="ms-comment">'.__('For example 1999', MS_TEXT_DOMAIN).'</span>
						</td>
					</tr>
					<tr>
						<td>
...
<?php            
		...
		*/
		public static function save_data(){
			...
			$file_path = trim( $_POST['ms_file_path'] );
			$demo_file_path = trim( $_POST['ms_demo_file_path'] );
			$cover = trim( $_POST['ms_cover'] );
